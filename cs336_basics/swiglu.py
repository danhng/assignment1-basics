...
class SwiGLU(nn.Module): 
    ...
    def __init__(self, d_ff, d_model, dtype, device, w1, w2, w3): 
        super().__init__()
        self.d_ff = d_ff
        self.d_model = d_model
        self.dtype = dtype
        self.device = device
        self.linear_1 = Linear(d_model, d_ff, device, dtype, w1)
        self.linear_3 = Linear(d_model, d_ff, device, dtype, w3)
        self.linear_2 = Linear(d_ff, d_model, device, dtype, w2)
    
    def forward(self, x): 
        h1 = self.linear_1.forward(x) # 1, d_model * d_model,d_ff = 1 * d_ff
        a1 = h1 * sigmoid(h1) # 1 * d_ff 
        h3 = self.linear_3.forward(x)  # 1 * d_ff todo: could we do h1 and h3 in parallel
        h2 = a1 * h3 # 1 * d_ff
        a2 = self.linear_2.forward(h2) # 1,d_ff * d_ff,d_model = 1,d_model
        return a2