...
def initPretoken(inputPath: str | os.PathLike, splitTokens: str, specialTokens: list[str], numProcesses=1, chunkSizeToProcess=1024*1024):
    ...
    with open(inputPath, "rb") as f:
        ...
        boundaries = find_chunk_boundaries(
            f, chunkSizeToProcess, bytes(splitTokens, "utf-8"))
        ...
        for start, end in tqdm(zip(boundaries[:-1], boundaries[1:]), desc="Init Pretoken"):
            ...
            splitTokenRegex = r"|" + \
                "|".join(re.escape(escapedToken)
                         for escapedToken in (delimTokens))
...
def processChunk(task):
    ...
    with open(inputPath, "rb") as f:
        f.seek(start)
        chunk = f.read(end - start).decode("utf-8", errors="ignore")
        splitTokenRegex = r"|"+"|".join(re.escape(escapedToken)
                                        for escapedToken in (delimTokens))
        chunk = re.sub(splitTokenRegex, "", chunk)
        if chunk:
            # remove special tokens
...
def initPretokenMultiProcess(inputPath: str | os.PathLike, splitTokens: str, specialTokens: list[str], chunkSizeToProcess=1024*1024, processCount=1):
    ...
    with open(inputPath, "rb") as f:
        ...
        for start, end in zip(boundaries[:-1], boundaries[1:]):
            tasks.append(
                (start, end, inputPath, splitTextToken, specialTokens))

        with multiprocessing.Pool(processes=processCount) as pool:
            # pool.map distributes the list items across worker processes
            iterMap = tqdm(pool.imap_unordered(processChunk, tasks),
                           total=len(boundaries)-1, desc="Init Pretoken")
    ...
    for word, wordCount in pretokens.items():
        ...
        for i in range(len(word) - 1):
            ...
            if initPairCache:
                ...
                # append count to handle to multi-pair in a word
                pairCache[targetPair][word][0] = wordCount
                # append count to handle to multi-pair in a word
                pairCache[targetPair][word][1] = pairCache[targetPair][word][1] + 1
...
def getMaxPairCountCache(pretokens, initPairCache=False, pairCache={}):
    ...
    if initPairCache:
        ...
        for targetPair, pairSpec in pairCache.items():
            pairCount = sum(pairSpecMap[0] * pairSpecMap[1]
                            for pairSpecMap in pairSpec.values())
...
def getMaxPairCountCache2(pretokens, initPairCache=False, pairCache={}, pairCacheSort=SortedList()):
    ...
    if initPairCache:
        ...
        for i in range(len(pairCacheSort)):
            # pop the candidate max pair from pairCacheSort
            candidateMaxPair = pairCacheSort[-1]
...
def mergePretokenCache(pretokens, maxPair, pairCache, pairSortedCache: SortedList):
    ...
    for word in wordsToProcess:
        ...
        while currentWordIndex < (len(word) - 1):
            checkingPretokenPair = tuple(
                [''.join(word[currentWordIndex]), ''.join(word[currentWordIndex+1])])
            logger.debug(
                f"checking pair {checkingPretokenPair} at index: {currentWordIndex} of word {word} ")
            if checkingPretokenPair == maxPair[0]:
                if currentWordIndex > 0:
                    # newOverlappingPairBehind = tuple([''.join(word[i-1]), ''.join(maxPair[0])]) # h, ow
                    newOverlappingPairBehind = tuple(
                        # h, ow
                        [''.join(newPretoken[newIndex-1]), ''.join(maxPair[0])])
                    oldAdjacentPair = tuple(
                        # h, o
                        [''.join(word[currentWordIndex-1]), ''.join(word[currentWordIndex])])
                    ...
                    if (checkNewIndex not in newIndicesProcessed):
                        ...
                        if newPretoken not in pairCache[newOverlappingPairBehind]:
                            pairCache[newOverlappingPairBehind][newPretoken] = [
                                pairCache[oldAdjacentPair][word][0], 0]
                        # only append 1 to occurences.
                        pairCache[newOverlappingPairBehind][newPretoken][1] = pairCache[newOverlappingPairBehind][newPretoken][1] + 1
                        newIndicesProcessed.add(checkNewIndex)
                        newPairsToSort.add(newOverlappingPairBehind)
                    else:
                        logger.debug(
                            f"oops: found processed overlapp pair behind at new index {checkNewIndex}, old pair {oldAdjacentPair}, new pair {newOverlappingPairBehind}")

                    if (checkCurrentIndex not in currentWordIndicesProcessed):
                        # subtract 1
                        pairCache[oldAdjacentPair][word][1] = pairCache[oldAdjacentPair][word][1] - 1
                        if pairCache[oldAdjacentPair][word][1] <= 0:
                            logger.debug(
                                f"old adjacent pair {oldAdjacentPair}, word {word} reaches 0 occurences, removing from pair cache")
                            # remove the old pair cache. Ex. {h, o -> {(h, o, w) -> 1, (h, o, t) -> 1}} => {h, o -> {(h, o, t) -> 1}}
                            del pairCache[oldAdjacentPair][word]
                        if not pairCache[oldAdjacentPair]:
                            logger.debug(
                                f"old adjacent pair {oldAdjacentPair} has no relevant word left, removing from pair cache")
                            del pairCache[oldAdjacentPair]
                        currentWordIndicesProcessed.add(checkCurrentIndex)
                        newPairsToSort.add(oldAdjacentPair)
                    else:
                        logger.debug(
                            f"oops: found processed overlapp pair behind at old index {checkCurrentIndex}, old pair {oldAdjacentPair}, new pair {newOverlappingPairBehind}")

                if currentWordIndex < len(word) - 2:
                    # newOverlappingPairAhead = tuple([''.join(maxPair[0]), ''.join(word[i+2])]) # ow, e
                    newOverlappingPairAhead = tuple(
                        # ow, e
                        [''.join(maxPair[0]), ''.join(newPretoken[newIndex+1])])
                    checkNewIndex = newIndex
                    oldAdjacentPair = tuple(
                        # w, e
                        [''.join(word[currentWordIndex+1]), ''.join(word[currentWordIndex+2])])
                    checkCurrentIndex = currentWordIndex+1
                    ...
                    if (checkNewIndex not in newIndicesProcessed):
                        ...
                        if newPretoken not in pairCache[newOverlappingPairAhead]:
                            pairCache[newOverlappingPairAhead][newPretoken] = [
                                pairCache[oldAdjacentPair][word][0], 0]
                        # only append 1 to occurences.
                        pairCache[newOverlappingPairAhead][newPretoken][1] = pairCache[newOverlappingPairAhead][newPretoken][1] + 1
                        newIndicesProcessed.add(checkNewIndex)
                        newPairsToSort.add(newOverlappingPairAhead)
                    else:
                        logger.debug(
                            f"oops: found processed overlapp pair ahead at new index {checkNewIndex}, old pair {oldAdjacentPair}, new pair {newOverlappingPairAhead}")
                    if (checkCurrentIndex not in currentWordIndicesProcessed):
                        # subtract 1
                        pairCache[oldAdjacentPair][word][1] = pairCache[oldAdjacentPair][word][1] - 1
                        if pairCache[oldAdjacentPair][word][1] <= 0:
                        ...
                        logger.debug(
                            f"oops: found processed overlapp pair behind at old index {checkCurrentIndex}, old pair {oldAdjacentPair}, new pair {newOverlappingPairAhead}")
                logger.debug(
                    f"found max pair in new pretoken {newPretoken} at index {currentWordIndex}")
            ...
            if (currentWordIndex >= len(word) - 2 or (currentWordIndex < len(word) - 2 and maxPair[0] != tuple([''.join(word[currentWordIndex+1]), ''.join(word[currentWordIndex+2])]))):
                ...
                logger.debug(
                    f"found normal non overlapping pair at cur index {currentWordIndex}: {checkingPretokenPair}")
                ...
                if newPretoken not in pairCache[checkingPretokenPair]:
                    pairCache[checkingPretokenPair][newPretoken] = [
                        pairCache[checkingPretokenPair][word][0], 0]
                ...
                if pairCache[checkingPretokenPair][word][1] <= 0:
                    logger.debug(
                        f"current un-adjacent {checkingPretokenPair} has no relevant with word {word} left, removing from pair cache")
                    del pairCache[checkingPretokenPair][word]
            currentWordIndex = currentWordIndex + 1
            newIndex = newIndex+1  # keep track of new index of new Pre token
        # add new preToken to pretokenMap
        pretokens[newPretoken] = pretokens[word]
    ...
    if len(pairSortedCache) > 0:
        ...
        for pairToSort in newPairsToSort:
            if (pairToSort in pairCache):
                pairCount = sum(pairSpecMap[0] * pairSpecMap[1]
                                for pairSpecMap in pairCache.get(pairToSort).values())
                pairSortedCache.add(tuple([pairToSort, pairCount]))

    # remove the old pair (as they are merged to one token), o, w -> {}
    del pairCache[maxPair[0]]
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    if get_init_multi_process:
        pretokens = initPretokenMultiProcess(inputPath=input_path, splitTokens=split_text_token,
                                             specialTokens=special_tokens, chunkSizeToProcess=chunk_size_to_process, processCount=process_count)
    else:
        pretokens = initPretoken(inputPath=input_path, splitTokens=split_text_token,
                                 specialTokens=special_tokens, chunkSizeToProcess=chunk_size_to_process)
    end_time1 = time.perf_counter()
    elapsed_time1 = end_time1 - start_time
    logger.info(
        f"Init pretokens: Vocab size: {vocab_size}, Elapsed time: {elapsed_time1:.4f} seconds")
    ...
    merges = []
    pairCache = {}
    # compare count first, then value for ties
    pairCacheSort = SortedList([], key=itemgetter(1, 0))
    ...
    for iteration in tqdm(range(vocab_size-len(vocab)), desc="Training vocab"):
        ...
        if get_max_by_cache:
            maxPair, pairCache, pairCacheSort = getMaxPairCountCache2(
                pretokens, iteration == 0, pairCache, pairCacheSort)
        else:
            maxPair, pairCache = getMaxPairCount(
                pretokens, iteration == 0, pairCache)
        if (maxPair[1] > 0):
            logger.info(f"iter {iteration}: max pair: {maxPair}")
            pretokens = mergePretokenCache(
                pretokens, maxPair, pairCache, pairCacheSort)
            logger.info(
                f"iter {iteration}: pretokens size: {len(pretokens)}, pair cache size: {len(pairCache)}")
            visual = ''.join(maxPair[0]).replace(
                chr(0x20), chr(288)).replace(chr(0x0A), chr(266))
            # word = word.replace(chr(0x20), chr(288))
            # word = word.replace(chr(0x0A), chr(266))
    ...
    logger.warning(
        f"Vocab size: {vocab_size}, Init pretoken time: {elapsed_time1:.1f} seconds")
